...
- **Ngày công KPI cá nhân** = ngày công chuẩn hiệu lực của kỳ − ngày phép đã duyệt của chính nhân viên trong tháng đó. `getApprovedLeaveDaysInMonth()` tính theo từng ngày (T2–T6 = 1.0, T7 = 0.5, CN/ngày lễ = 0), không trừ đơn chờ duyệt hoặc bị từ chối. `AdminKpiOtView` áp dụng theo từng nhân viên; `KpiRewardsView` áp dụng cho User. Override cũng là mẫu số để snapshot đơn giá OT khi tạo nháp KPI.
