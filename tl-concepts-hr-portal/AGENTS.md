...
- **Ngày công chuẩn của tháng** mặc định = (số ngày T2–T6 trong tháng × 1.0) + (số ngày T7 × 0.5) − (ngày lễ/Tết rơi vào T2–T7, quy đổi 1.0/0.5 tương ứng). Hàm gốc: `getMonthWorkDays(month, year, holidayDates)` trong `src/utils/workDays.ts` — hoạt động đúng cho bất kỳ tháng/năm nào (dùng `Date`, không hardcode lịch), holiday lấy từ `useCompanyHolidays()` (`company_holidays` table, quản lý ở màn Quản lý Ngày phép). HR/Kế toán hoặc Admin có thể ghi đè **cho đúng một tháng/năm** qua `company_workday_overrides`; không được đổi khi KPI kỳ đó đang chờ duyệt hoặc đã phát hành.
- **Ngày công KPI cá nhân** = ngày công chuẩn hiệu lực của kỳ − ngày phép đã duyệt của chính nhân viên trong tháng đó. `getApprovedLeaveDaysInMonth()` tính theo từng ngày (T2–T6 = 1.0, T7 = 0.5, CN/ngày lễ = 0), không trừ đơn chờ duyệt hoặc bị từ chối. `AdminKpiOtView` áp dụng theo từng nhân viên; `KpiRewardsView` áp dụng cho User. OT chỉ đồng bộ số giờ; tiền OT thực tế do HR/Kế toán nhập thủ công trong payroll.
- **Chỉ tiêu KPI tháng của 1 nhân viên** = `kpi_target_per_day × ngày công KPI cá nhân` (tính lại mỗi khi đổi tháng, thêm ngày lễ hoặc đơn phép chuyển sang `Đã duyệt`). KHÔNG còn định mức chung cho toàn công ty (`company_settings.kpi_rate_per_day` đã bỏ dùng — xem Lessons #4).
...
- `approve_payroll_month` và `retry_payslip_notification` dùng `INSERT ... ON CONFLICT DO UPDATE` để tạo/reset job email trên `notification_outbox`. Bảng outbox có RLS `SELECT/INSERT/UPDATE` chỉ cho **Admin đúng công ty**; không cấp cho HR/Kế toán hay User (`20260826133000_payroll_outbox_admin_update_policy.sql`).
