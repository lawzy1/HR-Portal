...
import { FileCheck, TrendingUp, Receipt, AlertTriangle, Plus, Pencil, Send, ShieldCheck, RotateCcw, Search, Eye, X, CalendarRange, Users, FileText } from 'lucide-react';
...
import { getContractCustomFields, getContractLifecycleStatus, latestContractsByEmployee } from '../../utils/contracts';
...
export const AdminContractSalaryView: React.FC = () => {
  ...
  const handleSelectEmployee = (employeeId: string) => {
    ...
    const element = document.getElementById('employee-contract-detail');
    if (element) {
  ...
  const handleCreateContract = () => {
    if (allEmployees[0] && !selectedId) {
      setSelectedEmployeeIdForAdmin(allEmployees[0].id);
    }
  ...
  return (
    <div className="space-y-8">
      {/* 1. Contract Directory Overview (Top Section) */}
      <ContractDirectory
      ...
      {selectedEmp ? (
        <div id="employee-contract-detail" className="space-y-6 pt-6 border-t border-slate-200">
          {/* Employee Detail Page Header */}
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">
                {t('contract.employeeHeader')}
              </h2>
              <p className="text-xs text-slate-500 mt-0.5">
                {t('contract.employeeSubheader')}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className="px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-sm"
              >
                <Plus className="w-4 h-4" /> {t('contract.createContract')}
              </button>
              <div className="flex items-center space-x-3 bg-slate-50 p-2 rounded-xl border border-slate-200">
                <span className="text-xs font-bold text-slate-500 uppercase px-1">{t('contract.selectEmployee')}</span>
                <SearchableSelect
          ...
          <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white rounded-2xl p-6 shadow-lg flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <div className="flex items-center space-x-3">
                <h3 className="text-xl font-extrabold">{selectedEmp.full_name}</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-left border-t md:border-t-0 md:border-l border-slate-700/80 pt-4 md:pt-0 md:pl-6">
              <div>
                <span className="text-[11px] uppercase font-bold text-slate-400 block">{t('contract.currentSalary')}</span>
              ...
              <div>
                <span className="text-[11px] uppercase font-bold text-slate-400 block">{t('contract.startDate')}</span>
                <span className="text-sm font-bold text-amber-300">{selectedEmp.start_date ? formatDate(selectedEmp.start_date) : '—'}</span>
              </div>
              <div>
                <span className="text-[11px] uppercase font-bold text-slate-400 block">{t('contract.lastReview')}</span>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h3 className="font-bold text-slate-900 text-base flex items-center space-x-2">
                  ...
                  <span>{t('contract.section1')}</span>
                </h3>
                <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200">{t('contract.type')}: {selectedEmp.contract_type ? translateValue(selectedEmp.contract_type) : '—'}</span>
              </div>

              <div className="space-y-3">
                {!contracts || contracts.length === 0 ? (
                  <div className="p-6 text-center text-slate-400 text-xs">{t('contract.empty')}</div>
                ) : (
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h3 className="font-bold text-slate-900 text-base flex items-center space-x-2">
                  ...
                  <span>{t('contract.section2')}</span>
                </h3>
              </div>

              <div className="space-y-3">
                {!salaryHistory || salaryHistory.length === 0 ? (
                  <div className="p-6 text-center text-slate-400 text-xs">{t('contract.noSalaryChange')}</div>
                ) : (
                  salaryHistory.map((sal) => (
                    <div key={sal.id} className="p-4 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-primary-700 bg-primary-50 px-2 py-0.5 rounded border border-primary-200">{translateValue(sal.change_type || '')}</span>
                        <span className="text-xs text-slate-500 font-medium">{t('contract.effectiveDate')}: {formatDate(sal.effective_date)}</span>
                      </div>
                      <div className="flex items-center justify-between pt-1 text-xs">
                        <div>
                          <span className="text-slate-500 block">{t('contract.oldSalary')}:</span>
                          <span className="line-through text-slate-400">{formatMoney(sal.old_salary || 0)}</span>
                        </div>
                        <div className="text-right">
                          <span className="text-slate-500 block">{t('contract.newSalary')}:</span>
                      ...
                      <p className="text-xs text-slate-600 pt-1 border-t border-slate-200/60">
                        {t('contract.reason')}: <span className="italic">{sal.reason}</span>{sal.approved_by ? ` (${t('contract.approver')}: ${getApproverDisplayName(sal.approved_by, allEmployees)})` : ''}
                      </p>
                    </div>
                  ))
                )}
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-primary-600">{t('contract.dirCategory')}</p>
        ...
        <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-3">
          ...
          <SummaryPill icon={<CalendarRange className="h-4 w-4" />} label={translateValue('Sắp hết hạn')} value={expiringCount} tone="amber" />
          <SummaryPill icon={<Users className="h-4 w-4" />} label={translateValue('Chờ duyệt')} value={pendingCount} tone="primary" />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full lg:max-w-md">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t('contract.searchPlaceholder')} className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm outline-none transition focus:border-primary-400 focus:bg-white focus:ring-4 focus:ring-primary-100" />
          </div>
          <div className="flex gap-1 overflow-x-auto rounded-xl bg-slate-100 p-1">
            {[
              ['all', translateValue('Tất cả')], ['active', translateValue('Hiệu lực')], ['expiring', translateValue('Sắp hết hạn')], ['pending', translateValue('Chờ duyệt')],
            ].map(([value, label]) => (
          ...
          <table className="w-full min-w-[920px] text-left">
            <thead className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className="px-4 py-3.5">{t('dashboard.period')}</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {filteredContracts.length === 0 ? (
                ...
                <tr key={contract.id} className="group hover:bg-primary-50/35">
                  ...
                  <td className="px-4 py-4"><span className="block font-bold text-slate-900">{contract.contract_code}</span><span className="mt-0.5 block text-xs text-slate-500">{contract.parent_contract_id ? `${t('contract.addendum')} · ` : ''}{translateValue(contract.type)}</span></td>
                  <td className="px-4 py-4 text-xs text-slate-600"><span className="block">{formatDate(contract.start_date)}</span><span className="mt-0.5 block text-slate-400">đến {contract.end_date ? formatDate(contract.end_date) : t('contract.noExpiry')}</span></td>
                  ...
                  <td className="px-4 py-4"><div className="flex justify-end gap-2"><button onClick={() => setPreview(contract)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs font-bold text-slate-600 hover:border-primary-200 hover:text-primary-700 cursor-pointer"><Eye className="h-3.5 w-3.5" /> {t('contract.quickView')}</button><button onClick={() => onOpen(contract.employee_id)} className="rounded-lg bg-slate-900 px-2.5 py-2 text-xs font-bold text-white hover:bg-slate-700 cursor-pointer">{t('contract.details')}</button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
...
const ApprovalBadge: React.FC<{ status: string }> = ({ status }) => {
  const { value: translateValue } = useI18n();
  const styles: Record<string, string> = {
    published: 'bg-teal-700 text-white border border-teal-800',
    pending_approval: 'bg-amber-600 text-white border border-amber-700',
    rejected: 'bg-rose-600 text-white border border-rose-700',
    draft: 'bg-slate-700 text-white border border-slate-800',
  };
  return (
    <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-extrabold shadow-2xs ${styles[status] || 'bg-slate-700 text-white'}`}>
      {translateValue(CONTRACT_APPROVAL_LABELS[status] || status)}
    </span>
  );
};
...
const ContractStatusBadge: React.FC<{ contract: DbContract }> = ({ contract }) => {
  ...
  const styles: Record<string, string> = {
    'Đang hiệu lực': 'bg-emerald-700 text-white border border-emerald-800',
    'Sắp hết hạn': 'bg-orange-600 text-white border border-orange-700',
  };
  return (
    <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-extrabold shadow-2xs ${styles[status] || 'bg-slate-600 text-white'}`}>
...
const ContractQuickPreview: React.FC<{ contract: DirectoryContract; onClose: () => void; onOpen: () => void }> = ({ contract, onClose, onOpen }) => {
  const { t, value: translateValue } = useI18n();
  const { formatMoney } = useMoneyVisibility();
  const customFields = getContractCustomFields(contract.custom_fields);
  return <div className="fixed inset-0 z-40 flex justify-end bg-slate-950/20 p-3 sm:p-5" onClick={onClose}>
    <aside className="flex h-full w-full max-w-md flex-col rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-start justify-between border-b border-slate-100 p-5"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-primary-600">{t('contract.quickView')}</p><h2 className="mt-1 text-lg font-extrabold text-slate-900">{contract.contract_code}</h2></div><button onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-800 cursor-pointer" aria-label="Close preview"><X className="h-5 w-5" /></button></div>
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm"><PreviewField label={t('contract.type')} value={translateValue(contract.type)} /><PreviewField label={t('contract.salary')} value={formatMoney(contract.salary || 0)} /><PreviewField label={t('contract.start')} value={formatDate(contract.start_date)} /><PreviewField label={t('contract.end')} value={contract.end_date ? formatDate(contract.end_date) : t('contract.noExpiry')} /></dl>
        {Object.keys(customFields).length > 0 && (
          <div className="rounded-xl border border-primary-100 bg-primary-50/50 p-4">
            <p className="mb-2 text-xs font-bold uppercase tracking-wide text-primary-700">Thông tin bổ sung</p>
            <dl className="space-y-2 text-sm">
              {Object.entries(customFields).map(([name, value]) => <PreviewField key={name} label={name} value={value || '—'} />)}
            </dl>
          </div>
        )}
        {contract.note && <div className="rounded-xl border border-slate-200 p-4 text-sm text-slate-600"><p className="mb-1 text-xs font-bold uppercase tracking-wide text-slate-400">{t('common.notes')}</p>{contract.note}</div>}
      ...
      <div className="border-t border-slate-100 p-5"><button onClick={onOpen} className="w-full rounded-xl bg-primary-600 px-4 py-3 text-sm font-bold text-white hover:bg-primary-700 cursor-pointer">{t('common.viewDetails')}</button></div>
    </aside>
  </div>;
};
...
const ContractCard: React.FC<{
  ...
  return (
    <div className={`p-5 rounded-2xl border space-y-4 shadow-xs transition-all ${
      isAddendum ? 'bg-primary-50/40 border-primary-200' : 'bg-white border-slate-200/90'
    }`}>
      {/* Card Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
        <div className="flex items-center space-x-2">
          {isAddendum && (
            <span className="text-[10px] font-extrabold px-2 py-0.5 bg-indigo-600 text-white rounded-md shadow-2xs">
              {t('contract.addendum')}
            </span>
          )}
        ...
        <div className="flex items-center gap-2">
          <ApprovalBadge status={contract.publish_status} />
          <ContractStatusBadge contract={contract} />

          {(contract.publish_status === 'draft' || contract.publish_status === 'rejected' || (isAdmin && contract.publish_status === 'published' && ['Đang hiệu lực', 'Sắp hết hạn'].includes(getContractLifecycleStatus(contract)))) && (
            <button onClick={onEdit} className="p-1.5 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 cursor-pointer text-slate-700 transition" title="Chỉnh sửa hợp đồng">
              <Pencil className="w-3.5 h-3.5" />
            </button>
          )}

          {(contract.publish_status === 'draft' || contract.publish_status === 'rejected') && (
            <button onClick={onSubmit} className="p-1.5 rounded-lg bg-primary-600 text-white hover:bg-primary-700 cursor-pointer shadow-xs transition" title="Gửi Admin duyệt">
          ...
          {isAdmin && contract.publish_status === 'pending_approval' && (
            <>
              <button onClick={onReject} className="p-1.5 rounded-lg border border-rose-300 bg-white text-rose-700 hover:bg-rose-50 cursor-pointer transition" title="Trả lại">
                <RotateCcw className="w-3.5 h-3.5" />
              </button>
              <button onClick={onApprove} className="p-1.5 rounded-lg bg-success-600 text-white hover:bg-success-700 cursor-pointer shadow-xs transition" title="Duyệt & áp dụng">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
          <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.type')}</span>
          <span className="font-bold text-slate-900 mt-0.5 block">{translateValue(contract.type)}</span>
        </div>

        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
          <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.position')}</span>
          <span className="font-bold text-slate-900 mt-0.5 block">{contract.position || '—'}</span>
        </div>

        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
          <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.signedDate')}</span>
          <span className="font-bold text-slate-900 mt-0.5 block">{contract.signed_date ? formatDate(contract.signed_date) : '—'}</span>
        </div>

        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
          <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('dashboard.period')}</span>
          <span className="font-bold text-slate-900 mt-0.5 block">
            {formatDate(contract.start_date)} đến {contract.end_date ? formatDate(contract.end_date) : t('contract.noExpiry')}
          </span>
        </div>
      </div>

      {/* Grid 2: Income & Benefits */}
      <div className="space-y-1.5">
        <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 px-1">Lương & Quyền lợi tài chính</span>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
          <div className="p-2.5 rounded-xl bg-success-50/70 border border-success-200/70">
            <span className="block text-[10px] font-bold uppercase tracking-wider text-success-700">{t('contract.salary')}</span>
            <span className="font-black text-sm text-success-800 font-mono mt-0.5 block">{formatMoney(contract.salary || 0)}</span>
          </div>

          {contract.allowance_amount > 0 && (
            <div className="p-2.5 rounded-xl bg-success-50/70 border border-success-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-success-700">{t('contract.allowance')}</span>
              <span className="font-black text-sm text-success-800 font-mono mt-0.5 block">{formatMoney(contract.allowance_amount)}</span>
            </div>
          )}

          {contract.kpi_target_month != null && (
            <div className="p-2.5 rounded-xl bg-primary-50/70 border border-primary-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-primary-700">{t('contract.kpiDay')}</span>
              <span className="font-bold text-primary-800 mt-0.5 block">{contract.kpi_target_month} view/ngày</span>
            </div>
          )}

          {contract.level_title && (
            <div className="p-2.5 rounded-xl bg-primary-50/70 border border-primary-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-primary-700">{t('contract.level')}</span>
              <span className="font-bold text-primary-800 mt-0.5 block">{contract.level_title}</span>
            </div>
          )}

          {contract.commission_rate_per_view > 0 && (
            <div className="p-2.5 rounded-xl bg-primary-50/70 border border-primary-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-primary-700">Commission / view</span>
              <span className="font-bold text-primary-800 font-mono mt-0.5 block">{formatMoney(contract.commission_rate_per_view)}</span>
            </div>
          )}

          {contract.qc_commission_rate_per_view > 0 && (
            <div className="p-2.5 rounded-xl bg-primary-50/70 border border-primary-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-primary-700">QC Commission / view</span>
              <span className="font-bold text-primary-800 font-mono mt-0.5 block">{formatMoney(contract.qc_commission_rate_per_view)}</span>
            </div>
          )}

          {contract.phone_allowance > 0 && (
            <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.phoneAllowance')}</span>
              <span className="font-bold text-slate-900 font-mono mt-0.5 block">{formatMoney(contract.phone_allowance)}</span>
            </div>
          )}

          {contract.lunch_allowance > 0 && (
            <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.lunchAllowance')}</span>
              <span className="font-bold text-slate-900 font-mono mt-0.5 block">{formatMoney(contract.lunch_allowance)}</span>
            </div>
          )}

          {contract.guaranteed_income > 0 && (
            <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200/70">
              <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">{t('contract.guaranteedIncome')}</span>
              <span className="font-bold text-slate-900 font-mono mt-0.5 block">{formatMoney(contract.guaranteed_income)}</span>
            </div>
          )}
        </div>
      </div>

      {/* Additional Details & File Attachment */}
      <div className="pt-2 border-t border-slate-100 text-xs space-y-2">
        {(contract.work_location || contract.working_schedule) && (
          <p className="text-slate-600">
            {contract.work_location && <span>Địa điểm: <b>{contract.work_location}</b></span>}
            {contract.work_location && contract.working_schedule && <span> • </span>}
            {contract.working_schedule && <span>Lịch làm việc: <b>{contract.working_schedule}</b></span>}
          </p>
        )}

        {isAddendum && contract.adjustment_categories.length > 0 && (
          <p className="text-slate-700 bg-primary-50/60 p-2 rounded-lg border border-primary-200/60">
            {t('contract.adjustments')} <b>{contract.adjustment_categories.join(', ')}</b>
          </p>
        )}

        {Object.keys(customFields).length > 0 && (
          <div className="rounded-xl border border-primary-100 bg-primary-50/50 p-3">
            <p className="mb-2 text-[10px] font-extrabold uppercase tracking-wider text-primary-700">Thông tin bổ sung</p>
            <dl className="grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-2">
              {Object.entries(customFields).map(([name, value]) => (
                <div key={name}>
                  ...
                  <dd className="mt-0.5 break-words font-semibold text-slate-700">{value || '—'}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        {contract.note && (
          <p className="text-slate-500 italic bg-slate-50 p-2 rounded-lg border border-slate-200/60">
            {t('common.notes')}: {contract.note}
          </p>
        )}

        {contract.rejection_reason && (
          <p className="font-semibold text-rose-700 bg-rose-50 p-2 rounded-lg border border-rose-200">
            Lý do trả lại: {contract.rejection_reason}
          </p>
        )}
      </div>
    </div>
