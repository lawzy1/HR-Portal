        ...
        <section className="flex flex-col justify-between p-7 sm:p-10 lg:p-14">
          <div>
            <div className="flex items-start justify-between gap-4">
              <a href="https://tlconceptsltd.com/" target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center gap-3">
            ...
            <form onSubmit={loginForm.handleSubmit(onLogin)} className="mt-16 space-y-4 ">
                ...
                <Field label={t('login.password')} error={loginForm.formState.errors.password?.message}>
                ...
                <SubmitButton loading={loginForm.formState.isSubmitting}>{t('login.submit')}</SubmitButton>
            </form>
        ...
        <section className="relative hidden lg:grid grid-cols-2 grid-rows-2 gap-1.5 bg-stone-900 p-1.5">
          ...
          <div className="absolute inset-x-8 bottom-8 rounded-2xl border border-white/20 bg-stone-950/55 p-5 text-white backdrop-blur-md">
            ...
            <a href="https://tlconceptsltd.com/our-works" target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-white hover:text-emerald-200">
              {t('login.explore')} <ArrowRight className="h-4 w-4" />
            </a>
          </div>
        </section>
      </div>
...
const PasswordToggle: React.FC<{ show: boolean; toggle: () => void }> = ({ show, toggle }) => {
  const { t } = useI18n();
  return (
    <button type="button" onClick={toggle} aria-label={t(show ? 'login.hidePassword' : 'login.showPassword')} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 cursor-pointer">
